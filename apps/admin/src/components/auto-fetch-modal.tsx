...
export function AutoFetchModal({ open, onClose, onDone }: Props) {
  ...
  const runImport = async () => {
    ...
    setProgressMeta({ done: 0, total: totalGroups, label: "Translating + importing…" });
    for (const [cat, articles] of grouped) {
      idx++;
      setProgress(`Importing ${idx} / ${totalGroups}: ${cat} (${articles.length} article${articles.length === 1 ? "" : "s"})`);
      setProgressMeta({
        done: idx - 1, total: totalGroups,
        label: `Importing ${cat} · ${articles.length} article${articles.length === 1 ? "" : "s"}`,
      });
      try {
        const res = await fetch("/api/auto-fetch", {
          method: "POST",
        ...
        if (!res.ok || !data) {
          ...
          acc.push({
            cat,
            fetched: row?.fetched ?? articles.length,
      ...
      setPerCategory([...acc]);
      // Increment AFTER work for this category completes — drives the bar
      // from 0% → 100% based on actual completions, not dispatches.
      setProgressMeta({
        done: idx, total: totalGroups,
        label: `Imported ${cat}`,
      });
    }
    // Hold 100% briefly so the user sees the bar fill before we close it.
    await new Promise((r) => setTimeout(r, 300));
    setRunning(false);
